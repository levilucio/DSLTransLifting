...
public class InstanceDatabase {
	// data information
	// keeps the input model that gets changed throughout the transformation
	...
	public InstanceDatabase clone(Hashtable<String, String> presenceConds) {
		...
		{{
			for(InstanceEntity ie : getLoadedClasses()) {
				...
				try {
					...
					// replace the reference of the cloned object in the presence condition database by the reference of its clone
					if (presenceConds.containsKey(Integer.toString(ie.hashCode()))) {
						//System.out.println("Replacing: " + Integer.toString(ie.hashCode()) + "in presence condition database");
						String presenceCond = presenceConds.get(Integer.toString(ie.hashCode()));
						presenceConds.remove(Integer.toString(ie.hashCode()));
						presenceConds.put(Integer.toString(cloned.hashCode()), presenceCond);				
					}
			...
			for(InstanceRelation ir : getLoadedRelations()) {
				...
				// replace the reference of the cloned relation in the presence condition database by the reference of its clone
				if (presenceConds.containsKey(Integer.toString(ir.hashCode()))) {
//					System.out.println("Replacing: " + Integer.toString(ir.hashCode()) + "in presence condition database");
					String presenceCond = presenceConds.get(Integer.toString(ir.hashCode()));
					presenceConds.remove(Integer.toString(ir.hashCode()));
					presenceConds.put(Integer.toString(cloned.hashCode()), presenceCond);				
				}
		}}}
		{
...
//			}
	...
	public InstanceEntity findInstanceEntity(String typename,String name) {
		for(InstanceEntity ie: getLoadedClasses()) {
			//System.out.println("----------------");
			//System.out.println("Type: " + ie.getDotNotation().replace("'",""));		
			//System.out.println("Type looked for: " + elementPresenceCond.get(0));
			//System.out.println("----------------");
			if (ie.getDotNotation().replace("'","").equals(typename)) {
				//System.out.println("Found correct loaded EMF object!");
				for(InstanceAttribute ia: getAttributesByInstanceEntity(ie)){
					...
					if (ia.getMetaAttribute().getName().equals("Name") && ia.getValue().equals(name)) 
